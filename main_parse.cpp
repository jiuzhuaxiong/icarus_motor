...
bool parseNote(char* in, uint8_t* note, uint8_t* duration, int8_t& size){
    uint8_t notes[] = {142, 127, 239, 213, 190, 179, 159}; // A B C D E F G
    uint8_t sharps[] = {134, 127, 225, 201, 179, 169, 150}; // A# B C# D# F F# G#
    uint8_t flats[] = {150, 134, 245, 225, 201, 190, 190}; //A^ B^ C^ D^ E^ E G^
...
int main() {
    ...
    PRINT_DEBUG("Starting parser");   
    thread_parser.start(parse_input_thread);
    PRINT_DEBUG("Size: %d", int(thread_parser.free_stack()));
    ...
    while (1){
        PRINT_DEBUG("Size: %d", int(thread_parser.used_stack()));
        // set_pwm(239);
        // PRINT_DEBUG("Tick: %d", tick);
        // Thread::wait(100);
