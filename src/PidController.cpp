...
// PidPosController::PidPosController(float k_p, float k_i, float k_d, float max_out /*=1.0*/) :
//   PidController(k_p, k_i, k_d, max_out)
// {
// }

// PidVelController::PidVelController(float k_p, float k_i, float k_d, float max_out /*=1.0*/) :
//   PidController(k_p, k_i, k_d, max_out)
// {
// }
...
float PidController::computeOutput(float reference, float measurement, float dt)
{
  ...
  last_de_dt_ = de_dt;
   ...
   // PRINT_DEBUG("Measure: %d.%03d Err: %de-9 P: %de-9 I: %de-9 D: %de-9 Ref: %d.%03d Out: %d.%03d",
