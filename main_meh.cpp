...
volatile float pwm_duty_cycle = 0.2;
...
volatile int target_v;

void vel_control_thread(){
    float k = 0.00001;
    int err;
    while(1){
        err = target_v-velocity;
        pwm_duty_cycle += k*err;
        Thread::wait(100);
    }
}
...
int main() {
    ...
    PRINT_DEBUG("Used: %d/%d",thread_spin.used_stack(),thread_spin.max_stack());

    thread_v.start(velocity_thread);
    // thread_vel_control.start(vel_control_thread);
    ...
    while (1){
        ...
        PRINT_DEBUG("Duty: 0.%03d",(int)(pwm_duty_cycle*1000)%1000);
        Thread::wait(1000);
        // set_pwm(213);
        // PRINT_DEBUG("Vel from Tick: %d.%03d",velocity/1000,abs(velocity%1000));
        // PRINT_DEBUG("Ticks: %d",tick)
